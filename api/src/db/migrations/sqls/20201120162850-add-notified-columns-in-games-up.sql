ALTER TABLE games ADD COLUMN notified_start timestamp, ADD COLUMN notified_end timestamp;
DROP VIEW game_players_view;
DROP VIEW games_all_infos;
CREATE OR REPLACE VIEW public.games_all_infos
 AS
 ...
 CREATE OR REPLACE VIEW public.game_players_view
  AS
    ...
    FROM ( SELECT user_entity_role.user_id,
              user_entity_role.entity_id
            FROM user_entity_role
            WHERE user_entity_role.role = 1) t4
      JOIN (( SELECT team_players.person_id,
              team_players.roster_id AS rost_id,
              team_players.role
            FROM team_players) t3
      JOIN ( SELECT games_all_infos.id AS game_id,
            ...
            FROM games_all_infos
              JOIN ( SELECT game_teams.roster_id,
                      game_teams.game_id
